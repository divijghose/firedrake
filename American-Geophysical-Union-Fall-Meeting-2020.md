...
Firedrake is an automated system for the solution of partial differential equations (PDEs). It enables to write down PDE-based problems, like the ones occuring across the geophysical sciences,  in a highly productive way while relying on automatic code generation to have high performance simulations. Many software packages have been built on top of it such as Thetis, Gusto or Icepack. One of the limitation of high level domain specific languages for FEM based simulations is that they do not take into account operators that are not directly expressible in the vector calculus sense. This limitation is critical in many applications for geophysical sciences where PDEs are not enough to accurately describe the physical problem of interest. For example, one can think of nonlinear implicit constitutive laws such as the Glen's flow law for glacier flow, or more recently the use of neural neural network to include features not represented in the differential equations, or to include closures for unresolved scales. Examples of neural network use include regularizer for inverse problems such as the one arising in seismic inversion and  subgrid parameterization for oceanographic applications. We present extensions to Firedrake that enable the inclusion of arbitrary external operators. The external operator feature composes seamlessly with the automatic differentiation capabilities of Firedrake.
