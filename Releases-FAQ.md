...
<< PETSc just do a merge. Should we be more rigorous? >>
...
Major Firedrake releases are made roughly every 6 months. The releases are timed to happen shortly after PETSc makes a minor release (e.g. ``3.22.5 -> 3.23.0``) such that any breaking API changes in the release may be fixed.
