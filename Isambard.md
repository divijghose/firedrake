...
* At the `spack install` step, Spack may complain about a missing dependency `gnuconfig` for the `libffi` package. It will tell you to add `depends_on('gnuconfig', type='build', when='@3.4.2')` to the package definition (exact version may not be `3.4.2`). The package file will be located in `spack/var/spack/repos/builtin/packages/libffi/package.py`. You can see how to add dependencies from the [example in the Spack documentation](https://spack-tutorial.readthedocs.io/en/latest/tutorial_packaging.html#adding-dependencies).
