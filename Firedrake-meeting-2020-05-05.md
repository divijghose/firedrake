...
## DH Complex Sprint
Now passing in almost all cases - 1 bug (where a test function and trial function get mixed up) needs to be fixed. **Action** DH to find time to fix to get final test passing. Noted that real mode could be improved with better implementation of PETSc real matrices.
Merging plan discussed. **Action** Someone to look at TSFC merge. **Action** DH to find time to sort out Jenkins to run in real and complex mode.
